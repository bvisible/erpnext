				...
				if (
					...
					if flat_rate and (tax_excluded or self.get("discount_amount")):
						if item.item_tax_template:
							vat_details = frappe.db.get_all(
								"Item Tax Template Detail",
								{"parent": item.item_tax_template, "parenttype": "Item Tax Template", "tax_rate": [">", 0]},
								["tax_type", "tax_rate"]
							)
							if vat_details and vat_details[0].tax_type in wanted_vat:
								applied_tax_rate = vat_details[0].tax_rate
								base_amount += base_amount * applied_tax_rate / 100
								amount += amount * applied_tax_rate / 100
					#////
