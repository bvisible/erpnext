...
class PurchaseInvoice(BuyingController):
	...
	if TYPE_CHECKING:
		...
		#//// Neoffice — the auto-generated type block below was RE-WRAPPED by hand (33e5705d01,
		#//// 2025-11-05, "…and purchase invoice type hints and UI tweaks"): upstream splits these long
		#//// imports over three lines each and puts `from frappe.types import DF` first. Purely
		#//// cosmetic, but this block is regenerated by `bench build-types`, so the next regeneration
		#//// will revert it and every upstream merge will conflict here. Take upstream's formatting and
		#//// keep only the two real changes marked below (status Literal, supplier_group).
		...
		#//// Neoffice — "In Payment Run" added to the status list (e84ce61bd4, 2025-11-28), together
		#//// with the field in purchase_invoice.json: a submitted, still-outstanding invoice flagged
		#//// is_proposed shows as being in a payment run instead of Unpaid/Overdue. Set in set_status()
		#//// at the end of this file and coloured in purchase_invoice_list.js.
		...
		#//// Neoffice — supplier_group is a Custom Field of ours on Purchase Invoice; the line keeps the
		#//// generated type block in step with the DocType as it exists on our sites.
	...
	def make_item_gl_entries(self, gl_entries):
		...
		#//// Neoffice — Swiss FLAT-RATE VAT (taux de dette fiscale nette) in the item GL entries. There
		#//// is no upstream equivalent: upstream books net amounts and lets the tax table carry the VAT.
		#//// Under the flat-rate method the input VAT is NOT reclaimed, so the expense must be booked
		#//// VAT-included and the VAT tax rows must not be posted at all (see make_tax_gl_entries below).
		#//// Lineage: 8fc9863753 + d2edcd9978 (2024-04-12, "updates to fix accounting vat" / "restore
		#//// state after fixing flat rate vat issues"), then read from the per-company doctype instead
		#//// of a global setting by 3e1125d454 and c9526355cc (2026-04-21, "read vat_accounting_method /
		#//// vat settings from Neoffice Company Settings"). `wanted_vat` collects the tax account heads
		#//// that carry a `tax_code` (our erpnextswiss Custom Field on Account).
		#//// Cross-app dependency: erpnext now reads the `Neoffice Company Settings` doctype, which
		#//// erpnext does not ship — guard or move this out at the merge.
		#//// TO REVIEW: `company = frappe.defaults.get_global_default("company")` is dead — the code
		#//// below uses self.company throughout.
		#//// added block
		...
		#////
		for item in self.get("items"):
			#//// Neoffice — added guard + block (8fc9863753 / d2edcd9978, 2024-04-12): builds the `remark`
							...
							self.get_gl_dict(
								{
									...
									"cost_center": item.cost_center,
									"project": item.project or self.project,
									#//// Neoffice — remarks on the expense GL entry; upstream leaves the field to get_gl_dict's
									#//// default (the invoice remarks). See the `remark` block marker above.
	...
	#//// Neoffice — flat-rate VAT, tax side (same lineage as the item block above: 8fc9863753 /
	#//// d2edcd9978 2024-04-12, then 3e1125d454 / c9526355cc 2026-04-21). Under the Swiss flat-rate
	#//// method input VAT is not reclaimable, so a tax row whose account carries a `tax_code` is
	#//// SKIPPED entirely (`continue`) — its amount is already inside the expense booked above.
	#//// Upstream posts every tax row. The `remark` built here goes on the tax GL entry, same
	#//// reason as on the item side. `company = frappe.defaults.get_global_default("company")` is
	#//// dead here too (TO REVIEW).
					...
					self.get_gl_dict(
						{
							...
							#//// Neoffice — remarks on the tax GL entry (see the marker on make_tax_gl_entries above).
							"remarks": remark, #//// added
						},
						account_currency,
						item=tax,
	...
	def on_cancel(self):
		...
		self.ignore_linked_doctypes = (
			...
			#//// Neoffice — three COMMENTED-OUT entries follow, i.e. they are inert: they were tried in the
			#//// ignore_linked_doctypes tuple of on_cancel and left in place. TO REVIEW: delete them, or
			#//// state why cancelling a Purchase Invoice would have to ignore its Payment / Journal Entry
			#//// links — doing so would let a paid invoice be cancelled without unlinking its payment.
			#"Payment Entry", #//// added
			#"Journal Entry", #//// added
	...
	def set_status(self, update=False, status=None, update_modified=True):
		...
		if not status:
			if self.docstatus == 2:
				...
				#//// Neoffice — added branch (e84ce61bd4, 2025-11-28): a submitted invoice flagged is_proposed
				#//// (our Custom Field, set when the invoice enters a payment run) shows "In Payment Run"
				#//// instead of Overdue/Partly Paid/Unpaid, so the accountant does not chase an invoice already
				#//// queued for payment. Placed BEFORE the is_overdue test on purpose.
